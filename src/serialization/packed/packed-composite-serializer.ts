...
export default class PackedCompositeSerializer<ObjectType extends KeyProvider> implements CompositeSerializer<ObjectType, EncoderSequence> {

    private readonly encoder = new ArrayEncoder();
    private readonly decoder = new ArrayDecoder();

    readonly serializers: Map<string, Serializer<ObjectType, EncoderSequence>> = new Map();

    add(key: string, serializer: Serializer<ObjectType, EncoderSequence>): this {
    ...
    deserialize(value: EncoderSequence, options: SerializationOptions): ObjectType {
