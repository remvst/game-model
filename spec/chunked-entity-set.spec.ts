...
describe("a chunked entity set", () => {
    ...
    beforeEach(() => {
        originalSet = new WatchableObjectSet(
            new ObjectSet(
                (entity) => entity.id,
    ...
    class TestTrait extends Trait {
        ...
        surfaceProvider: TraitSurfaceProvider = rectangleSurface(
            (trait, out) => {
                out.update(
                    this.rectangle.minX,
                    this.rectangle.minY,
    ...
    it("will not chunk anything by default", () => {
        const entity1 = testEntity(new Rectangle(-100000, -100000, 1, 1));
        originalSet.add(entity1);

        const entity2 = testEntity(new Rectangle(100000, 100000, 1, 1));
    ...
    it("will add entities if they're outside the chunked rectangle if they have a trait that ", () => {
        ...
        chunked.visibleRectangleProvider = (visible, relevant) => {
            visible.update(
                entity.position.x + 50000,
                entity.position.y + 50000,
                2,
                2,
            );
            relevant.update(
